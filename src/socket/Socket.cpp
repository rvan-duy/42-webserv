...
void Socket::prepare() {
  ...
  if (_fd == -1) {
    // logger.error("[PREPARING] Socket: Failed to create socket: " +
    // std::string(strerror(errno)));   // double trouble?
    throw std::runtime_error("[PREPARING] Socket: Socket creation failed: " +
                             std::string(strerror(errno)));
  }

  /* Set options on socket to reuse address         */
  if (setsockopt(_fd, SOL_SOCKET, SO_REUSEADDR, &on, sizeof(on)) == -1) {
    logger.error("[PREPARING] Socket: Failed to set socket options: " +
                 std::string(strerror(errno)));
    throw std::runtime_error("[PREPARING] Socket: Socket options failed: " +
                             std::string(strerror(errno)));
  }
  ...
  if (ioctl(_fd, FIONBIO, (char *)&on) == -1) {
    logger.error("[PREPARING] Socket: Failed to set socket to nonblocking: " +
                 std::string(strerror(errno)));
    throw std::runtime_error("[PREPARING] Socket: Socket nonblocking failed: " +
                             std::string(strerror(errno)));
  }
  ...
  if (bind(_fd, (struct sockaddr *)&_servaddr, sizeof(_servaddr)) == -1) {
    logger.error("[PREPARING] Socket: Failed to bind socket: " +
                 std::string(strerror(errno)));
    throw std::runtime_error("[PREPARING] Socket: Socket bind failed: " +
                             std::string(strerror(errno)));
  }

  if (listen(_fd, BACKLOG) == -1) {
    logger.error("[PREPARING] Socket: Failed to prepare socket: " +
                 std::string(strerror(errno)));
    throw std::runtime_error("[PREPARING] Socket: Socket listen failed: " +
                             std::string(strerror(errno)));
  }
}
...
void Socket::addClient(const int &socket) {
  ...
  std::pair<HttpRequest *, Server *> newPair(NULL, NULL);
  _clients[socket] = newPair;
}

// TODO: fix segfault when printing request, might be done
void Socket::_addRequestToClient(int const &clientFd, HttpRequest *request,
                                 Server *server)  // TODO: CHUNKED support
{
  Logger::getInstance().log("Adding response to client with fd: " +
                            std::to_string(clientFd));
  std::pair<HttpRequest *, Server *> newPair(request, server);
  _clients[clientFd] = newPair;
}

void Socket::_addBadRequestToClient(
    const int &fd, HTTPStatusCode statusCode)  // TODO: CHUNKED support
{
...
bool Socket::hasClient(const int &fd) const { return _clients.count(fd) != 0; }

/*
 * Remove a client socket from the server
 */
void Socket::removeClient(const int socket) { _clients.erase(socket); }

int Socket::getPort() const { return _port; }

int Socket::getFd() const { return _fd; }

void Socket::addServer(Server const &server) { _servers.push_back(server); }

std::vector<Server> Socket::getServers() const { return _servers; }
