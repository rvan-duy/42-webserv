...
Parser::Parser(std::vector<Token> &tokens)
    : _tokens(tokens), _it(_tokens.begin()) {}
 ...
 */
Token::ETokenType Parser::lookAhead(std::vector<Token>::iterator it,
...
int Parser::parseServerDataLines(Server *pServer,
                                 std::vector<t_dataLine> const &lines) {
  for (size_t i = 0; i < lines.size(); i++) {
    for (size_t j = 0; j < PARSER_FUNC_N; j++) {
      // If keys match -> execute function
...
int Parser::parseLocationBlocks(Route *pRoute,
                                std::vector<t_dataLine> const &lines) {
  for (size_t i = 0; i < lines.size(); i++) {
...
int Parser::parseDataBlocks(Server *pServer,
...
/* Loops over datablocks to find blocks that start with "server", then converts
 * them*/
