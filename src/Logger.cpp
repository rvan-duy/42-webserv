...
void Logger::restart(void) { logToFile("[RESTART]", ""); }
...
void Logger::logToFile(std::string const &levelMsg,
...
void Logger::logToConsole(std::string const &levelMsg, ELogLevel level,
                          std::string const &message) {
  if (LOG_ENABLED == 0) {
    return;
  }
...
bool Logger::isFileOpen() const { return _file.is_open(); }
